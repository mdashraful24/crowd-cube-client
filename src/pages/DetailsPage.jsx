...
const DetailsPage = () => {
    ...
    useEffect(() => {
        fetch(`${import.meta.env.VITE_API_URL}/running/${id}`)
            .then((res) => res.json())
            .then((data) => {
                setCampaign(data);
                setLoading(false);
    ...
    useEffect(() => {
        if (user?.email && campaign?._id) {
            fetch(`${import.meta.env.VITE_API_URL}/myDonations?email=${user.email}`)
                .then((res) => res.json())
                .then((data) => {
    ...
    const handleDonate = () => {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/myDonations`, {
            method: "POST",
            headers: {
